...
- `stage_aruco.marker_size_mm`, `marker_ids`: 스테이지에 부착한 별도 마커 정보. 크기는 흰색 사각형 전체가 아니라 ArUco의 검은 외곽 사각형 한 변을 캘리퍼스로 잰 값입니다. 현재 스테이지 마커 실측값은 `12 mm`이므로 기본값은 `12.0`입니다. ChArUco 보정판 내부 마커의 `10 mm`와 혼동하면 안 됩니다.

현재 ChArUco 보정판은 13 mm 정사각형 22×15칸, 내부 코너 21×14 규격이며
기본 설정이 이 PDF와 일치합니다.
...
세션 폴더에 이전 촬영 결과가 있으면 삭제하거나 덮어쓰지 않고, 현재 시각이 붙은
새 세션 폴더를 자동으로 만들어 사용합니다.
한글이나 공백이 포함된 Windows 경로에서도 패턴과 촬영 이미지를 읽고 쓸 수 있습니다.
각 pose는 처음부터 최종 폴더에 기록하므로 폴더 이동이나 삭제 권한이 없어도 동작하며,
실패한 촬영은 진단을 위해 `rejected_poses` 항목에 남고 보정 계산에서는 제외됩니다.
패턴과 프로젝터 모니터 해상도가 다르면 메인 촬영 프로그램과 동일하게 최근접 보간으로
종횡비를 유지해 확대하고, 남는 영역은 검은색으로 채웁니다.
